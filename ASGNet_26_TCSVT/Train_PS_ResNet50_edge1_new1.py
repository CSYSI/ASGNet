...
from lib.Network_ResNet_ps_edge_1_new import Network
...
def train(train_loader, model, optimizer, epoch, save_path, writer):
    ...
    try:
        for i, (images, gts, edges) in enumerate(train_loader, start=1):
            ...
            loss_edge = dice_loss(preds[5], edges)*0.0625 + dice_loss(preds[6], edges)*0.125 + dice_loss(preds[7], edges)*0.25 + dice_loss(preds[8], edges)*0.5
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                print('{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} Loss1: {:.4f} Loss2: {:0.4f} LossE: {:0.4f}'.
                      format(datetime.now(), epoch, opt.epoch, i, total_step, loss.data, loss_init.data, loss_final.data,loss_edge.data)) # loss_edge.data
                logging.info(
                    '[Train Info]:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Total_loss: {:.4f} Loss1: {:.4f} Loss2: {:0.4f} loss2, LossE: {:0.4f}'.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decay_epoch', type=int, default=80, help='every n epochs decay learning rate')#80
    ...
    parser.add_argument('--save_path', type=str,default='./Network_ResNet50/', help='the path to save model and log')
